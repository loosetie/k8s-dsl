...
@K8sDslMarker
interface Prioritylevelconfigurationspec_flowcontrol_apiserver_k8s_io_v1beta2: K8sManifest {
  /** `limited` specifies how requests are handled for a Limited priority level. This field must be non-empty if and only if
`type` is `"Limited"`. */
  val limited: LimitedPriorityLevelConfiguration_flowcontrol_apiserver_k8s_io_v1beta2?
