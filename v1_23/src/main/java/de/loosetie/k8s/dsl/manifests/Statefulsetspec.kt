...
@K8sDslMarker
interface Statefulsetspec_apps_v1: K8sManifest {
  /** Minimum number of seconds for which a newly created pod should be ready without any of its container crashing for it to
be considered available. Defaults to 0 (pod will be considered available as soon as it is ready) This is an alpha field
...
enabled, which is alpha. +optional */
  val persistentVolumeClaimRetentionPolicy: StatefulSetPersistentVolumeClaimRetentionPolicy_apps_v1?
...
waiting, and on scale down will delete all pods at once. Possible enum values: - `"OrderedReady"` will create pods in
strictly increasing order on scale up and strictly decreasing order on scale down, progressing only when the previous
pod is ready or terminated. At most one pod will be changed at any time. - `"Parallel"` will create and delete pods as
soon as the stateful set replica count is changed, and will not wait for pods to be ready or complete termination. */
...
revision is made to Template. */
  val updateStrategy: StatefulSetUpdateStrategy_apps_v1?
...
over any volumes in the template, with the same name. */
  val volumeClaimTemplates: List<PersistentVolumeClaim_core_v1>?
}