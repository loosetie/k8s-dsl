...
@K8sDslMarker
interface Flowschemaspec_flowcontrol_apiserver_k8s_io_v1: K8sManifest {
...
that the distinguisher is disabled and thus will always be the empty string. */
  val distinguisherMethod: FlowDistinguisherMethod_flowcontrol_apiserver_k8s_io_v1?
...
resolved, the FlowSchema will be ignored and marked as invalid in its status. Required. */
  val priorityLevelConfiguration: PriorityLevelConfigurationReference_flowcontrol_apiserver_k8s_io_v1?
...
one member of rules matches the request. if it is an empty slice, there will be no requests matching the FlowSchema. */
  val rules: List<PolicyRulesWithSubjects_flowcontrol_apiserver_k8s_io_v1>?
}