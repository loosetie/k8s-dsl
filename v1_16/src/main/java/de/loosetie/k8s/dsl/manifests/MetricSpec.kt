...
@K8sDslMarker
interface MetricSpec_autoscaling_v2beta2: K8sManifest {
...
or QPS from loadbalancer running outside of cluster). */
  val external: ExternalMetricSource_autoscaling_v2beta2?
  /** object refers to a metric describing a single kubernetes object (for example, hits-per-second on an Ingress object). */
  val `object`: ObjectMetricSource_autoscaling_v2beta2?
...
transactions-processed-per-second). The values will be averaged together before being compared to the target value. */
  val pods: PodsMetricSource_autoscaling_v2beta2?
...
scaling options on top of those available to normal per-pod metrics using the "pods" source. */
  val resource: ResourceMetricSource_autoscaling_v2beta2?
