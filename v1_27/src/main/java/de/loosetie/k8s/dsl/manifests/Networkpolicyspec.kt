...
@K8sDslMarker
interface Networkpolicyspec_networking_k8s_io_v1: K8sManifest {
...
default). This field is beta-level in 1.8 */
  val egress: List<NetworkPolicyEgressRule_networking_k8s_io_v1>?
...
solely to ensure that the pods it selects are isolated by default) */
  val ingress: List<NetworkPolicyIngressRule_networking_k8s_io_v1>?
...
podSelector matches all pods in this namespace. */
  ...
  /** policyTypes is a list of rule types that the NetworkPolicy relates to. Valid options are ["Ingress"], ["Egress"], or
["Ingress", "Egress"]. If this field is not specified, it will default based on the existence of ingress or egress
rules; policies that contain an egress section are assumed to affect egress, and all policies (whether or not they
contain an ingress section) are assumed to affect ingress. If you want to write an egress-only policy, you must
explicitly specify policyTypes [ "Egress" ]. Likewise, if you want to write a policy that specifies that no egress is
allowed, you must specify a policyTypes value that include "Egress" (since such a policy would not include an egress
section and would otherwise default to just [ "Ingress" ]). This field is beta-level in 1.8 */
