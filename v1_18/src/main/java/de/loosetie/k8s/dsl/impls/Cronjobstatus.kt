package de.loosetie.k8s.dsl.impls

import com.fasterxml.jackson.annotation.JsonIgnore
import com.fasterxml.jackson.annotation.JsonPropertyOrder
import de.loosetie.k8s.dsl.HasParent
import de.loosetie.k8s.dsl.K8sManifest
import de.loosetie.k8s.dsl.manifests.Cronjobstatus_batch_v1beta1_k8s1_18
import de.loosetie.k8s.dsl.manifests.ObjectReference_core_v1_k8s1_18
import de.loosetie.k8s.dsl.manifests.Time_meta_v1_k8s1_18

@JsonPropertyOrder("active", "lastScheduleTime")
class Cronjobstatus_batch_v1beta1_k8s1_18Impl(
  @JsonIgnore
  override val parent: K8sManifest? = null
)
: Cronjobstatus_batch_v1beta1_k8s1_18, HasParent {
  override var active: List<ObjectReference_core_v1_k8s1_18>? = null
  override var lastScheduleTime: Time_meta_v1_k8s1_18? = null
}